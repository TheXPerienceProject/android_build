...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/xperiencee/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function destroy_build_var_cache()
{
    ...
    local v
    for v in $(echo "$cached_vars"); do
      unset var_cache_$v
    done
    unset cached_vars
    for v in $(echo "$cached_abs_vars"); do
...
function generate_hidl_vendor_qcom()
{
