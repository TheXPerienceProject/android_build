...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/xperience/build/envsetup.sh | tr '()' '  ' | $one_true_awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | grep -vE "^(print|COMMON_LUNCH_CHOICES)$" | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/xperience/build/envsetup.sh | tr '()' '  ' | $one_true_awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | grep -vE "^(print|COMMON_LUNCH_CHOICES)$" | sort -u | tr '\n' ' '`)
...
function _lunch_meat()
{
    ...
    local prebuilt_kernel_headers=$(_get_build_var_cached TARGET_PREBUILT_KERNEL_HEADERS)
    if [[ "$no_kernel" == "true" ]] || [ -n "$prebuilt_kernel" ]; then
        ...
        if [ -n "$prebuilt_kernel_headers" ]; then
            export TARGET_PREBUILT_KERNEL_HEADERS="$prebuilt_kernel_headers"
        fi
    else
...
if [ -z "${CCACHE_EXEC}" ]; then
    ...
    if command -v ccache &>/dev/null; then
        ...
        if [ -f "${CACHE_CONFIG_FILE}" ]; then
            CUSTOM_CACHE_DIR=$(cat "${CACHE_CONFIG_FILE}" | tr -d '\r\n')
        ...
        # Create CCACHE_DIR if it doesn't exist
        if [ ! -d "${CCACHE_DIR}" ]; then
            mkdir -p "${CCACHE_DIR}" && echo "Created CCACHE_DIR at: ${CCACHE_DIR}" >&2
        fi
